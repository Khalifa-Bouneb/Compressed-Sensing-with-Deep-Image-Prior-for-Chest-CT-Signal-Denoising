...
class Concat(nn.Module):
    ...
    def forward(self, input):
        ...
        # NEW TORCH.COMPILE-FRIENDLY VERSION:
        target_shape2 = min(inputs_shapes2)
        target_shape3 = min(inputs_shapes3)

        same_shape2 = all(shape == target_shape2 for shape in inputs_shapes2)
        same_shape3 = all(shape == target_shape3 for shape in inputs_shapes3)

        if same_shape2 and same_shape3:
            inputs_ = inputs
        else:
            inputs_ = []
            for inp in inputs:
